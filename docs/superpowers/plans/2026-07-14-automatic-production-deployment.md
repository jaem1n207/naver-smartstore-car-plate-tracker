...
**Current pull request:** Draft PR #2 on `naver-smartstore-car-plate-tracker-mvp`. Implementation and independent code, security, operator, and CI reviews are complete. The legacy `/opt/naver-smartstore-car-plate-tracker` checkout must move to `/srv/carplate-bootstrap-source` before bootstrap, and PR #2 must use a merge commit so its bootstrapped head remains an ancestor of `main`. Squash/rebase merge is prohibited for this migration. Only the documented one-time Oracle/GitHub setup and production confirmation remain operator-owned work.
...
- Normal deployment moves only forward through fetched `origin/main`; equal or stale requests are successful no-ops and divergent history fails closed. Because bootstrap seeds PR #2's head, PR #2 must use a merge commit rather than squash/rebase merge.
...
- [x] **Step 3: Implement atomic lock acquisition and release**
...
- [x] **Step 4: Add the locked sync facade and configuration**
...
- [x] **Step 5: Write and pass integration and CLI tests**
...
- [x] **Step 3: Extract the scheduler controller**
...
- [x] **Step 4: Wire production main and revision logging**
...
- [x] **Step 5: Add real child-process shutdown coverage**
...
- [x] **Step 4: Implement the durability helper**
...
- [x] **Step 5: Add script validation and package contract**
...
- [x] **Step 1: Write failing entrypoint and deployment integration tests**
...
- [x] **Step 2: Write failing recovery and isolation tests**
...
- [x] **Step 3: Implement forced entrypoint and monotonic state machine**

`deploy-entrypoint.sh` accepts only exact `SSH_ORIGINAL_COMMAND`, clears the environment, and invokes the single sudo-approved deployer. `deploy.sh` uses a root-owned `flock`, fetches only the compiled-in GitHub HTTPS origin, requires the requested SHA to equal the current `origin/main` tip, rechecks that tip immediately before activation, and returns older/equal requests as no-op.
...
- [x] **Step 4: Implement isolated candidate build and sealing**

Run `build-candidate.sh` through `systemd-run --wait --collect` as `carplate-build` with `KillMode=control-group`, bounded runtime/memory/swap/tasks/file size, `ProtectSystem=strict`, `NoNewPrivileges=true`, and candidate/package-store-only writable paths. The fetch phase pins the npm registry, disables lifecycle scripts, and denies private, loopback, and link-local destinations. The install/build phase uses the fetched cache with `PrivateNetwork=true`.
...
- [x] **Step 5: Implement durable activation, health, and recovery**
...
- [x] **Step 6: Implement repeat-safe bootstrap and hardened units**
...
- [x] **Step 7: Pass the full deployment suite**
...
- [x] **Step 1: Install only necessary latest-minor packages**
...
- [x] **Step 3: Add the pinned workflow and governance files**

Use reviewed full SHA pins for checkout, setup-node, pnpm setup, and upload-artifact. Download versioned actionlint and shellcheck binaries from their official releases and verify fixed SHA-256 digests before use. The `verify` job runs frozen install, Playwright Chromium install, `pnpm test:all`, `pnpm build`, shell syntax/static checks, and generated CSS diff. Upload Playwright artifacts for seven days on failure. The deploy job has a timeout exceeding the 60-minute drain plus 30-minute build and returns only allowlisted server output.
...
- [x] **Step 4: Make visual tests deterministic**
...
- [x] **Step 5: Verify CI and visual contracts**
...
- [x] **Step 1: Write the repeat-safe migration runbook**

Document, in execution order: backup/current-state checks; stop and move the legacy `/opt/naver-smartstore-car-plate-tracker` checkout to `/srv/carplate-bootstrap-source`; fast-forward the latest PR branch; persistent 2 GiB swap verification; Node 22.23.1 and pnpm 11.10.0; dedicated users; `/opt` release layout; `/etc` secret migration; Google credential permissions; reviewed script installation; deploy-key generation; root-owned authorized key; SSH fingerprint verification; sudoers validation; hardened systemd units; GitHub `production` environment secrets; branch protection; merge-commit-only handling for PR #2; first initialization; `workflow_dispatch`; reboot verification; rollback drill; deploy-key rotation; and diagnostic commands.
...
- [x] **Step 2: Align existing documentation**
...
- [x] **Step 3: Run security and secret scans**
...
- [x] **Step 5: Perform independent final reviews**
...
- [x] **Step 6: Complete the plan ledger and commit**

Mark all task checkboxes complete and commit the plan/docs with message `Document automatic deployment operations`.

- [x] **Step 7: Push the existing PR branch**

Push `naver-smartstore-car-plate-tracker-mvp`, confirm Draft PR #2 contains all commits, and report the exact one-time Oracle/GitHub steps the user must perform before merging.
