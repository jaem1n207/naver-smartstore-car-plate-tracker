...
4. Confirm all seven Korean tabs were created automatically and their first rows are frozen.
5. Confirm `원본 데이터` receives rows with Korean headers.
6. Confirm `차량번호 추출 실패` has image-only or no-text products.
7. Confirm `양쪽 스토어 중복` and `스토어 내부 중복` match known cases.
...
If legacy English tabs already exist, confirm they were renamed in place. If both an English and Korean version existed before the run, the worker preserves both and writes only to the Korean tab.
